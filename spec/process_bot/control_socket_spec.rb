...
describe ProcessBot::ControlSocket do
  it "increases the port if already in use" do
    ...
    control_socket2 = ProcessBot::ControlSocket.new(options: ProcessBot::Options.new(port: 7086), process: process2)
    control_socket2.start

    expect(control_socket2).to have_attributes(port: 7087)
  ensure
